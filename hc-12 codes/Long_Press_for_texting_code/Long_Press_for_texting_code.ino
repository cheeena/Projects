...
void setup() {
  ...
  message = String("suck my ass");
  Serial.setTimeout(35);                       // Open serial port to computer
  HC12.setTimeout(35);
}

void loop() {
 if(HC12.available()){                     // If Arduino's HC12 rx buffer has data
    Serial.println(HC12.readString());              // Send the data to the computer
    }
  if(Serial.available()){                   // If Arduino's computer rx buffer has data
    HC12.print(Serial.readString());              // Send that data to serial
  }
}
